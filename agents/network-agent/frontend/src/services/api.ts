...
class ApiService {
  ...
  private formatSearchParams(params: SearchParams): Record<string, any> {
    ...
    if (params.filters.timeRange) {
      formatted.startTime = params.filters.timeRange.start;
      formatted.endTime = params.filters.timeRange.end;
    }

    if (params.filters.severity) {
      formatted.severity = params.filters.severity.join(',');
    }

    if (params.filters.protocols) {
      formatted.protocols = params.filters.protocols.join(',');
    }

    if (params.filters.sources) {
      formatted.sources = params.filters.sources.join(',');
    }

    if (params.filters.destinations) {
      formatted.destinations = params.filters.destinations.join(',');
    }

    if (params.filters.alertTypes) {
      formatted.alertTypes = params.filters.alertTypes.join(',');
    }
